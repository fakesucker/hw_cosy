#!/usr/bin/env bash
#
# infer_shenhu_sft_ckpt_sweep_init_bistream_0529_stream_up.sh
# ---------------------------------------------------------------
# 0529：扫 run_all_sft_init_ckpt_seq_8gpu_bigbatch_stream.sh 产出的 7 组 SFT（bistream bigbatch）。
# 每组取最近 TOP_N 个 epoch_*_whole.pt（默认 TOP_N=5），流式推理 + 客服 TN。
# 默认 BISTREAM_FIXED_RATIO=1：LLM inference_bistream [5:15]（与训练 branch_mode=bistream 对齐）。
# STREAM=1 仅控制 Flow 音频分包；文本侧由 --bistream_fixed_ratio 驱动。
#
# 7 个实验目录（默认 CKPT_ROOT=/home/node62_data/hkxie/ckpt/huawei/cosyvoice2）：
#   sft_xiaoyuzhou_init_dpo_ep5_bistream_bigbatch
#   sft_xiaoyuzhou_init_dpo_ep4_bistream_bigbatch
#   sft_xiaoyuzhou_init_f03_1e5_ep2_bistream_bigbatch
#   sft_xiaoyuzhou_init_f03_1e5_ep3_bistream_bigbatch
#   sft_xiaoyuzhou_init_shenhu_1e6_ep2_bistream_bigbatch
#   sft_xiaoyuzhou_init_shenhu_1e6_ep3_bistream_bigbatch
#   sft_xiaoyuzhou_init_base_ep0_s40k_bistream_bigbatch
#
...
#
# 仅跑某一实验：
#   ONLY_EXP=dpo_ep5 ./infer_shenhu_sft_ckpt_sweep_init_bistream_0529_stream_up.sh
#
set -euo pipefail
...
# --- defaults (0529 init-bistream sweep) ---
...
BISTREAM_TEXT_CHUNK_TOKENS="${BISTREAM_TEXT_CHUNK_TOKENS:-0}"
...
OUTPUT_BASE="${OUTPUT_BASE:-/home/work_nfs23/hkxie/hw_proj/CosyVoice/testout/test_init_bistream_0529_stream}"
...
if [[ -n "${MODEL_DIRS_OVERRIDE:-}" ]]; then
  ...
  MODEL_DIRS=(
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_dpo_ep5_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_dpo_ep4_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_f03_1e5_ep2_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_f03_1e5_ep3_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_shenhu_1e6_ep2_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_shenhu_1e6_ep3_bistream_bigbatch"
    "${CKPT_ROOT}/sft_xiaoyuzhou_init_base_ep0_s40k_bistream_bigbatch"
  )
fi
...
echo "============================================================"
echo "[sweep-0529] init-bistream SFT x7 | TOP_N=${TOP_N} (latest epochs)"
echo "[sweep-0529] CKPT_ROOT=${CKPT_ROOT}"
echo "[sweep-0529] RUN_MODE=${RUN_MODE} GPU_LIST=${GPU_LIST} PROCS_PER_GPU=${PROCS_PER_GPU}"
echo "[sweep-0529] META=${META_FILE} OUT=${RUN_OUTPUT_BASE}"
echo "[sweep-0529] STREAM=${STREAM} BISTREAM_FIXED_RATIO=${BISTREAM_FIXED_RATIO} BISTREAM_FIRST_BLOCK_TEXT_TOKENS=${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}"
echo "[sweep-0529] CS_TN=${COSYVOICE_CUSTOMER_SERVICE_TN:-0} IS_SFT=${IS_SFT} SFT_SPK_ID=${SFT_SPK_ID}"
echo "============================================================"
