...
def _tts_speech_token_tensor_to_list(token: torch.Tensor) -> list:
    """与传入 token2wav(token=...) 的序列一致（整型 speech token）。"""
    return [int(x) for x in token.flatten().detach().cpu().tolist()]
...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        missing_keys, unexpected_keys = self.llm.load_state_dict(torch.load(llm_model, map_location=self.device, weights_only=True), strict=False)
        ...
        with self.llm_context, torch.cuda.amp.autocast(self.fp16 is True and hasattr(self.llm, 'vllm') is False):
            if isinstance(text, Generator):
                ...
                token_generator = self.llm.inference_bistream(text=text,
                                                              ...
                                                              first_block_text_tokens=getattr(self, 'bistream_first_block_text_tokens', 0))
            else:
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            ...
            yield {
                ...
                'tts_speech_token': _tts_speech_token_tensor_to_list(this_tts_speech_token),
            }
        else:
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def __init__(self,
        ...
        self.hift = hift
        self.fp16 = fp16
        # NOTE: must match training static_chunk_size (prompt padding grid & steady streaming hop = 1s @ 25 token/s).
        self.token_hop_len = 25
        # First streaming chunk: speech tokens (before pre_lookahead) for lower latency; steady chunks use token_hop_len.
        self.stream_first_speech_tokens = 16
        self.token_max_hop_len = 4 * self.token_hop_len
        self.stream_scale_factor = 2
        assert self.stream_scale_factor >= 1, 'stream_scale_factor should be greater than 1, change it according to your actual rtf'
        # First TTS text block in inference_bistream: 0 => mix_ratio[0] (default 5), else e.g. 6 or 10 for [N:15].
        self.bistream_first_block_text_tokens = 0
        # hift cache
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            token_offset = 0
            steady_hop = self.token_hop_len
            prompt_token_pad = int(np.ceil(flow_prompt_speech_token.shape[1] / steady_hop) * steady_hop - flow_prompt_speech_token.shape[1])
            while True:
                time.sleep(0.1)
                this_token_hop_len = self.stream_first_speech_tokens + prompt_token_pad if token_offset == 0 else steady_hop
