...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        flow_matching_sft_ckpt = os.environ.get(
            'COSYVOICE2_FLOW_CHECKPOINT',
            "/home/work_nfs22/xmren/code/CosyVoice/examples/libritts/cosyvoice2/exp/flow/epoch_4_whole.pt",
        )
        # flow_matching_sft_ckpt = "/home/work_nfs14/code/kaohe_2025/kaohe_2025_rnyou/code/cosyvoice-kefu_finetune/exp/F03/flow/torch_ddp/epoch_4_whole.pt"
        if flow_matching_sft_ckpt and os.path.exists(flow_matching_sft_ckpt):
            logging.info("load CosyVoice2 flow checkpoint override: {}".format(flow_matching_sft_ckpt))
            self.model.load('{}/llm.pt'.format(model_dir),
                            flow_matching_sft_ckpt,
                            '{}/hift.pt'.format(model_dir))
        elif os.environ.get('COSYVOICE2_FLOW_CHECKPOINT'):
            raise FileNotFoundError("COSYVOICE2_FLOW_CHECKPOINT not found: {}".format(flow_matching_sft_ckpt))
        else:
            logging.warning("legacy CosyVoice2 flow checkpoint not found, keep default flow.pt: {}".format(
                flow_matching_sft_ckpt))
