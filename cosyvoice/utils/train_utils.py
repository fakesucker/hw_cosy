...
def batch_forward(model, batch, scaler, info_dict, ref_model=None, dpo_loss=None, distill_manager=None):
    ...
    with autocast:
        if distill_manager is not None and distill_manager.enabled is True and info_dict.get('tag') == 'TRAIN':
            info_dict['loss_dict'] = distill_manager.forward(model, batch, device, info_dict)
        else:
        ...
        if (info_dict['train_engine'] == 'deepspeed' and info_dict['is_gradient_accumulation_boundary'] is True) or \
           (info_dict['train_engine'] == 'torch_ddp' and (info_dict['batch_idx'] + 1) % info_dict['accum_grad'] == 0):
            ...
            for k, v in loss_dict.items():
                # TensorBoard add_scalar only accepts scalar values.
                # Some DPO intermediates (e.g. chosen_logps/rejected_logps) are vectors.
                if isinstance(v, torch.Tensor):
                    if v.numel() == 1:
                        scalar_v = v.item()
                    else:
                        scalar_v = v.detach().float().mean().item()
                else:
                    scalar_v = v
                writer.add_scalar('{}/{}'.format(tag, k), scalar_v, step + 1)
        ...
        for name, value in loss_dict.items():
            if isinstance(value, torch.Tensor):
                if value.numel() == 1:
                    ...
                    show_v = value.detach().float().mean().item()
            else:
                show_v = value
            log_str += '{} {:.6f} '.format(name, show_v)
        if tag == "TRAIN":
