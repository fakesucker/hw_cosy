...
class Qwen2LM(TransformerLM):
    def __init__(
        ...
        self.speech_token_size = speech_token_size
        if branch_mode not in ['auto', 'unistream', 'bistream']:
            raise ValueError('branch_mode must be one of [auto, unistream, bistream]')
        ...
        # Lazy-init: avoid 8-way ONNX CUDA session creation during hyperpyyaml load (CUBLAS race).
        self._speech_token_extractor = None
        self._speech_tokenizer_onnx = (
            os.path.join(onnx_path, 'speech_tokenizer_v2.batch.onnx') if online_feature is True else None
        )

    def _get_speech_token_extractor(self):
        if self._speech_token_extractor is None and self._speech_tokenizer_onnx is not None:
            self._speech_token_extractor = SpeechTokenExtractor(model_path=self._speech_tokenizer_onnx)
        return self._speech_token_extractor
    ...
    def prepare_lm_input_target(self, sos_emb, text_token, text_token_emb, text_token_len, task_id_emb,
                                ...
                                instruct_token_emb=None, instruct_token_len=None, branch_mode: str = 'auto',
                                branch_choices: Optional[List[bool]] = None, return_branch_choices: bool = False):
        lm_target, lm_input = [], []
        text_token = unpad_sequence(text_token, text_token_len.cpu(), batch_first=True)
        speech_token = unpad_sequence(speech_token, speech_token_len.cpu(), batch_first=True)
        ...
        if branch_mode not in ['auto', 'unistream', 'bistream']:
            raise ValueError('branch_mode must be one of [auto, unistream, bistream]')
        if branch_choices is not None and len(branch_choices) != len(text_token):
            raise ValueError('branch_choices length {} does not match batch size {}'.format(len(branch_choices), len(text_token)))
        branch_choices_out = []
        ...
        for i in range(len(text_token)):
            ratio_ok = speech_token_len[i] / text_token_len[i] > self.mix_ratio[1] / self.mix_ratio[0]
            if branch_choices is not None:
                use_bistream = bool(branch_choices[i]) and ratio_ok
            elif branch_mode == 'unistream':
                use_bistream = False
            elif branch_mode == 'bistream':
                use_bistream = ratio_ok
            else:
                use_bistream = (random.random() < 0.5) and ratio_ok
            branch_choices_out.append(bool(use_bistream))
            # bistream sequence
            if use_bistream:
        ...
        if return_branch_choices is True:
            return lm_target, lm_input, lm_input_len, branch_choices_out
        return lm_target, lm_input, lm_input_len

    def prepare_prompt_lm_input_target(self, sos_emb, prompt_text_emb, prompt_text_len, text_token_emb, text_token_len,
                                       task_id_emb, prompt_speech_token, prompt_speech_token_emb,
                                       prompt_speech_token_len, speech_token, speech_token_emb, speech_token_len):
        lm_target, lm_input = [], []
        prompt_text_emb = unpad_sequence(prompt_text_emb, prompt_text_len.cpu(), batch_first=True)
        text_token_emb = unpad_sequence(text_token_emb, text_token_len.cpu(), batch_first=True)
        ...
        speech_token = unpad_sequence(speech_token, speech_token_len.cpu(), batch_first=True)
        speech_token_emb = unpad_sequence(speech_token_emb, speech_token_len.cpu(), batch_first=True)
        for i in range(len(text_token_emb)):
            prompt_speech_len = int(prompt_speech_token[i].size(0))
            this_lm_target = torch.tensor(
                [IGNORE_ID] * (1 + prompt_text_emb[i].size(0) + text_token_emb[i].size(0) + prompt_speech_len) +
                speech_token[i].tolist() + [self.eos_token]
            )
            ...
            if this_lm_target.size(0) != this_lm_input.size(0):
                raise ValueError('prompt-conditioned lm_target length {} != lm_input length {}'.format(
                    this_lm_target.size(0), this_lm_input.size(0)))
            lm_target.append(this_lm_target)
            lm_input.append(this_lm_input)
        lm_input_len = torch.tensor([i.size(0) for i in lm_input], dtype=torch.int32)
        lm_input = pad_sequence(lm_input, batch_first=True, padding_value=IGNORE_ID)
    ...
    def forward_logits(
            ...
            batch: dict,
            device: torch.device,
            branch_choices: Optional[List[bool]] = None,
            speech_token_override: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        ...
        if speech_token_override is not None:
            speech_token, speech_token_len = speech_token_override
            speech_token = speech_token.to(device)
            speech_token_len = speech_token_len.to(device)
        elif 'speech_token' not in batch:
            speech_token, speech_token_len = self._get_speech_token_extractor().inference(
        ...
        if prompt_conditioned and self.__class__.__name__ != 'Qwen2LM':
            raise NotImplementedError('prompt_conditioned forward_logits currently supports Qwen2LM only')
        if self.__class__.__name__ == 'CosyVoice3LM':
            ...
            lm_target, lm_input, lm_input_len, branch_choices_out = self.prepare_lm_input_target(
                ...
                return_branch_choices=True,
            )
        elif self.__class__.__name__ == 'Qwen2LM':
            if prompt_conditioned:
                for key in ['prompt_text_token', 'prompt_text_token_len',
                            'prompt_speech_token', 'prompt_speech_token_len']:
                    if key not in batch:
                        raise ValueError('prompt_conditioned forward_logits requires batch[{}]'.format(key))
                if branch_choices is not None and any(branch_choices):
                    raise ValueError('prompt_conditioned forward_logits requires unistream branch choices')
                prompt_text_token = batch['prompt_text_token'].to(device)
                prompt_text_token_len = batch['prompt_text_token_len'].to(device)
                prompt_text_emb = self.llm.model.model.embed_tokens(prompt_text_token)
                prompt_speech_token = batch['prompt_speech_token'].to(device)
                prompt_speech_token_len = batch['prompt_speech_token_len'].to(device)
                prompt_speech_token_emb = self.speech_embedding(prompt_speech_token)
                lm_target, lm_input, lm_input_len = self.prepare_prompt_lm_input_target(
                    sos_emb, prompt_text_emb, prompt_text_token_len, text_token_emb, text_token_len,
                    task_id_emb, prompt_speech_token, prompt_speech_token_emb, prompt_speech_token_len,
                    speech_token, speech_token_emb, speech_token_len,
                )
                branch_choices_out = [False] * text_token.size(0)
            else:
                lm_target, lm_input, lm_input_len, branch_choices_out = self.prepare_lm_input_target(
                    sos_emb, text_token, text_token_emb, text_token_len, task_id_emb,
                    speech_token, speech_token_emb, speech_token_len,
        ...
        # 4. run lm forward
        if hasattr(lm_input, 'is_inference') and lm_input.is_inference():
            # Online OPSD sampling uses inference-mode generation before this
            # train forward. Qwen saves hidden states for backward, so its
            # input embeddings must be normal autograd tensors.
        ...
        valid_mask = lm_target != IGNORE_ID
        speech_mask = valid_mask & (lm_target >= 0) & (lm_target < self.speech_token_size)
        if speech_mask.any():
            speech_ce_loss = F.cross_entropy(
                logits.float().reshape(-1, logits.size(-1))[speech_mask.reshape(-1)],
                lm_target.reshape(-1)[speech_mask.reshape(-1)].long(),
                reduction='mean',
            )
        else:
            speech_ce_loss = loss.detach() * 0.0
        return {
            'loss': loss,
            'loss_ce': loss,
            'acc': acc,
            ...
            'speech_ce_loss': speech_ce_loss.detach(),
            'branch_choices': branch_choices_out,
        }
    ...
    def forward(
        ...
        forward_dict = self.forward_logits(batch, device)
        loss = forward_dict['loss']
        acc = forward_dict['acc']
        return {'loss': loss, 'acc': acc}
    ...
    def forward_dpo(
        ...
        # DPO / forced bi-stream SFT: text:speech blocks per mix_ratio (default 5:15).
        lm_target, lm_input, lm_input_len = self.prepare_lm_input_target(
            sos_emb, text_token.repeat(2, 1), text_token_emb.repeat(2, 1, 1), text_token_len.repeat(2),
    ...
    def inference_bistream(
            ...
            max_token_text_ratio: float = 20,
            min_token_text_ratio: float = 2,
            first_block_text_tokens: int = 0,
    ) -> Generator[torch.Tensor, None, None]:

        device = prompt_text.device
        first_block_text_tokens = first_block_text_tokens if first_block_text_tokens > 0 else self.mix_ratio[0]
        tts_first_block_pending = first_block_text_tokens > self.mix_ratio[0]
        # 1. prepare input
        if self.__class__.__name__ == 'CosyVoice3LM':
            sos_emb = self.speech_embedding.weight[self.sos].reshape(1, 1, -1)
            task_id_emb = self.speech_embedding.weight[self.task_id].reshape(1, 1, -1)
        ...
        def _tts_text_block_size():
            if tts_first_block_pending:
                return first_block_text_tokens
            return self.mix_ratio[0]

        def _try_append_tts_text_for_decode():
            nonlocal tts_first_block_pending, text_cache, lm_input
            block_size = _tts_text_block_size()
            if text_cache.size(1) < block_size:
                return False
            lm_input_text = text_cache[:, :block_size]
            logging.info('append {} tts text token (block={})'.format(lm_input_text.size(1), block_size))
            if len(out_tokens) != 0 and out_tokens[-1] == self.fill_token:
                lm_input = lm_input_text
            else:
                lm_input = torch.concat([lm_input, lm_input_text], dim=1)
            text_cache = text_cache[:, block_size:]
            if tts_first_block_pending:
                tts_first_block_pending = False
            return True
        ...
        for this_text in text:
            ...
            if prompt_speech_token_emb.size(1) == 0:
                if (len(out_tokens) != 0 and out_tokens[-1] == self.fill_token) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
                    ...
                    if not _try_append_tts_text_for_decode():
                        logging.info('not enough text token to decode, wait for more')
                        continue
                while True:
                    ...
                    if top_ids >= self.speech_token_size:
                        if top_ids == self.fill_token:
                            ...
                            print(f"top ids = {top_ids}, self.speech_token_size={self.speech_token_size}")
                            break
...
class CosyVoice3LM(Qwen2LM):
    def __init__(
        ...
        self._speech_token_extractor = None
        self._speech_tokenizer_onnx = (
            os.path.join(onnx_path, 'speech_tokenizer_v3.batch.onnx') if online_feature is True else None
        )
