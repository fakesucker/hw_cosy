...
# - 流式：STREAM=1 时 CosyVoice2/3 在 model 内固定首包 16 speech token、之后每包 token_hop_len（默认 25）
#
set -euo pipefail
...
OUTPUT_BASE="${OUTPUT_BASE:-/home/work_nfs23/hkxie/hw_proj/CosyVoice/testout/test_flowsft_0512_stream_sentence}"
...
echo "[sweep] META=${META_FILE} OUT=${RUN_OUTPUT_BASE} TOP_N=${TOP_N} CS_TN=${COSYVOICE_CUSTOMER_SERVICE_TN:-0} STREAM=${STREAM} STREAM_TEXT_INPUT=${STREAM_TEXT_INPUT} SENTENCE_PSEUDO_STREAM=${SENTENCE_PSEUDO_STREAM}"
echo "============================================================"
