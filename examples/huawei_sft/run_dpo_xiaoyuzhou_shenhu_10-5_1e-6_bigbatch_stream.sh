...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  echo "[NOTE] DPO LLM training: full-sequence unistream (forward_dpo branch_mode=unistream), not inference streaming."
  ...
  train_list="${dpo_data_root}/dpo_train_1000.list"
  cv_list="${dpo_data_root}/dpo_cv.list"
  init_sft_ckpt="${latest_ckpt:-/home/work_nfs23/hkxie/ckpt/huawei/cosyvoice2/dpo_xiaoyuzhou_shenhu_10-5_1e-6_bigbatch/epoch_5_whole.pt}"
  output_dir="/home/work_nfs23/hkxie/ckpt/huawei/cosyvoice2/dpo_xiaoyuzhou_shenhu_10-5_1e-6_bigbatch_stream"

  if [[ ! -f "${train_list}" || ! -f "${cv_list}" ]]; then
    echo "❌ DPO list not found under ${dpo_data_root}"
    exit 1
  fi
  ...
  for model in llm; do
    torchrun --nnodes=1 --nproc_per_node=$num_gpus \
      ...
      --config conf/cosyvoice2_dpo_1e-6_spk.yaml \
