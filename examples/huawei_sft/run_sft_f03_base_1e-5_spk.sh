...
# F03-only SFT (SPK): lr=1e-5, init from CosyVoice2 base llm.pt (no shenhu ckpt).
# Parallel layout (run_all_sft_4_parallel_f03_only_spk.sh): GPU 2, port 12755.
# . ./path.sh || exit 1;
...
BASE_CKPT=${pretrained_model_dir}/llm.pt
MODEL_DIR=/home/work_nfs23/hkxie/ckpt/huawei/cosyvoice2/sft_f03_base_only_1e-5_from_llm
dist_backend="nccl"
num_workers=1
prefetch=200
# prefetch=0
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  echo "✅ Using checkpoint: ${BASE_CKPT}"
