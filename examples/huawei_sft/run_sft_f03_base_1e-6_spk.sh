...
# F03-only SFT (SPK): lr=1e-6, init from CosyVoice2 base llm.pt (no shenhu ckpt).
# Parallel layout (run_all_sft_4_parallel_f03_only_spk.sh): GPU 3, port 12756.
# . ./path.sh || exit 1;
...
MODEL_DIR=/home/work_nfs23/hkxie/ckpt/huawei/cosyvoice2/sft_f03_base_only_1e-6_from_llm
dist_backend="nccl"
num_workers=1
prefetch=200
# prefetch=0
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for model in llm; do
    torchrun --nnodes=1 --nproc_per_node=$num_gpus \
        --rdzv_id=$job_id --rdzv_backend="c10d" --rdzv_endpoint="localhost:12756" \
