...
# DPO LLM only (--model llm --dpo). Data: dpo_train.list -> jsonl with code + reject_code (see
# data_list/dpo_process_filter/gemini_understanding/convert_kefu_pairs_to_cosy_dpo.py).
# DPO LLM uses branch_mode=bistream in forward_dpo (mix_ratio [5,15], same as SFT bi-stream).
# Inference: infer_seed.py --bistream_fixed_ratio (fixed 5 text tokens/chunk -> inference_bistream).
# vs run_dpo_*_bigbatch.sh: only output_dir suffix differs; yaml chunk_size=25 applies to Flow (not trained here).
# Override via env (used by run_all_sft_init_ckpt_seq_8gpu_bigbatch_stream.sh):
#   INIT_SFT_CKPT, SFT_OUTPUT_DIR, TRAIN_JOB_ID, TRAIN_BRANCH_MODE (default bistream)
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  echo "[NOTE] SFT LLM train_branch_mode=${train_branch_mode} (bistream=no random 50%% mix; uses mix_ratio blocks when ratio ok)"
  # /home/node62_data/hkxie/data/dpo_ready/dpo_train_1000.list
  # dpo_data_root="/home/node62_data/hkxie/data/dpo_ready"
  # train_list="${dpo_data_root}/dpo_train.list"
  ...
  init_sft_ckpt="${INIT_SFT_CKPT:-${latest_ckpt:-/home/work_nfs23/hkxie/ckpt/huawei/cosyvoice2/dpo_xiaoyuzhou_shenhu_10-5_1e-6_bigbatch/epoch_5_whole.pt}}"
  output_dir="${SFT_OUTPUT_DIR:-/home/node62_data/hkxie/ckpt/huawei/cosyvoice2/sft_xiaoyuzhou_shenhu_10-5_1e-6_bistream_bigbatch}"
  ...
  for model in llm; do
    torchrun --nnodes=1 --nproc_per_node=$num_gpus \
        --rdzv_id=$job_id --rdzv_backend="c10d" --rdzv_endpoint="localhost:12458" \
      ../../cosyvoice/bin/train.py \
      --train_engine $train_engine \
      --config conf/cosyvoice2_sft_1e-6_spk.yaml \
