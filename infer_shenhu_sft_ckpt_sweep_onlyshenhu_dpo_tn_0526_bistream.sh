#!/usr/bin/env bash
#
# infer_shenhu_sft_ckpt_sweep_onlyshenhu_dpo_tn_0526_bistream.sh
# -----------------------------------------------
...
# - STREAM=1 仅控制音频分包输出（首包16个speech token，后续token_hop_len=25）
# - 文本输入模式三选一：
#   1) BISTREAM_FIXED_RATIO=1      -> inference_bistream；首块 [N:15] + 后续 [5:15]
#   2) STREAM_TEXT_INPUT=1         -> 可变 chunk 文本流（iter_stream_text_chunks）
#   3) SENTENCE_PSEUDO_STREAM=1    -> 按标点切句，多次独立 inference
#
# 首块比例测试（保守）：
#   BISTREAM_FIRST_BLOCK_TEXT_TOKENS=6   -> 首块 [6:15]，后续 [5:15]
#   BISTREAM_FIRST_BLOCK_TEXT_TOKENS=10  -> 首块 [10:15]，后续 [5:15]
#   BISTREAM_FIRST_BLOCK_TEXT_TOKENS=0   -> 全程 [5:15]
#
# BISTREAM_FIXED_RATIO=1 \
# BISTREAM_FIRST_BLOCK_TEXT_TOKENS=6 \
# STREAM=1 \
...
BISTREAM_TEXT_CHUNK_TOKENS="${BISTREAM_TEXT_CHUNK_TOKENS:-0}"   # 0 => follow llm.mix_ratio[0] (default 5)
BISTREAM_FIRST_BLOCK_TEXT_TOKENS="${BISTREAM_FIRST_BLOCK_TEXT_TOKENS:-6}"  # 0/6/10 => [5:15]/[6:15]/[10:15] first block
BISTREAM_FIRST_CHUNK_TEXT_TOKENS="${BISTREAM_FIRST_CHUNK_TEXT_TOKENS:-0}"  # 0 => follow first block size
...
OUTPUT_BASE="${OUTPUT_BASE:-/home/work_nfs23/hkxie/hw_proj/CosyVoice/testout/test_flowsft_0527_bistream}"
META_FILE="${META_FILE:-/home/work_nfs23/hkxie/hw_proj/CosyVoice/data_list/kefu_0506_onlymale.lst}"
META_TAG="$(basename "${META_FILE}")"
META_TAG="${META_TAG%.*}"

MODEL_DIRS=(
  # "${CKPT_ROOT}/sft_top500k_shenhu_filter_wer_10-5_1e-5_spk"
  # "${CKPT_ROOT}/sft_top500k_shenhu_filter_wer_10-5_1e-6_spk"
  "${CKPT_ROOT}/dpo_xiaoyuzhou_shenhu_10-5_1e-6_bigbatch_stream"
  "${CKPT_ROOT}/dpo_xiaoyuzhou_shenhu_10-5_1e-6_bigbatch"
  "${CKPT_ROOT}/sft_shenhu_filter_wer_only_1e-5_from_llm"
)
...
echo "[sweep] META=${META_FILE} OUT=${RUN_OUTPUT_BASE} TOP_N=${TOP_N} CS_TN=${COSYVOICE_CUSTOMER_SERVICE_TN:-0}"
echo "[sweep] STREAM=${STREAM} BISTREAM_FIXED_RATIO=${BISTREAM_FIXED_RATIO} BISTREAM_FIRST_BLOCK_TEXT_TOKENS=${BISTREAM_FIRST_BLOCK_TEXT_TOKENS} BISTREAM_TEXT_CHUNK_TOKENS=${BISTREAM_TEXT_CHUNK_TOKENS}"
echo "[sweep] STREAM_TEXT_INPUT=${STREAM_TEXT_INPUT} SENTENCE_PSEUDO_STREAM=${SENTENCE_PSEUDO_STREAM}"
echo "============================================================"
...
# infer_seed.py 中这三种文本输入路径互斥，这里提前校验，避免跑起来才报错。
text_mode_count=0
...
if [[ ! "${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}" =~ ^[0-9]+$ ]]; then
  echo "[ERROR] BISTREAM_FIRST_BLOCK_TEXT_TOKENS must be a non-negative integer, got: ${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}"
  exit 1
fi
if [[ "${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}" != "0" && "${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}" != "6" && "${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}" != "10" && "${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}" != "7" ]]; then
  echo "[ERROR] BISTREAM_FIRST_BLOCK_TEXT_TOKENS must be 0, 6, or 10, got: ${BISTREAM_FIRST_BLOCK_TEXT_TOKENS}"
  exit 1
fi

if [[ ! "${BISTREAM_TEXT_CHUNK_TOKENS}" =~ ^[0-9]+$ ]]; then
  echo "[ERROR] BISTREAM_TEXT_CHUNK_TOKENS must be a non-negative integer, got: ${BISTREAM_TEXT_CHUNK_TOKENS}"
  exit 1
fi
if [[ ! "${BISTREAM_FIRST_CHUNK_TEXT_TOKENS}" =~ ^[0-9]+$ ]]; then
  echo "[ERROR] BISTREAM_FIRST_CHUNK_TEXT_TOKENS must be a non-negative integer, got: ${BISTREAM_FIRST_CHUNK_TEXT_TOKENS}"
  exit 1
fi

if [[ -n "${MODEL_DIRS_OVERRIDE}" ]]; then
  IFS=',' read -r -a MODEL_DIRS <<< "${MODEL_DIRS_OVERRIDE}"
fi
